...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname parameterization-starter-my-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (filter2 fun lon)
  (cond [(empty? lon) empty]
        [else
         (if (fun (first lon))
             (cons (first lon)
                   (filter2 fun (rest lon)))
             (filter2 fun (rest lon)))]))
