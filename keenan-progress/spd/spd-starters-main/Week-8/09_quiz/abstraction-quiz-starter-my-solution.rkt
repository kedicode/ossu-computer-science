...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname abstraction-quiz-starter-my-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
;  PROBLEM 1:
;
...
;(define (arrange-all how loi) empty-image) ;stub
(define (arrange-all how loi)
  (cond [(empty? loi) empty-image]
        [else
         (how (first loi)
              (arrange-all how (rest loi)))]))
...
(define (minus-n lon n)
  (cond [(empty? lon) empty]
        [else
         (local [(define (subn el n)
                   (- el n))]
         (cons (subn (first lon) n)
               (minus-n (rest lon) n)))]))

(define (minus-n lon n)
  (local [(define (subn el)
            (- el n))]
    (map subn lon)))
